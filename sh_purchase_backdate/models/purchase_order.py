...
class PurchaseOrder(models.Model):
    ...
    #     journal = self.env['account.move'].with_context(default_move_type=move_type)._search_default_journal()
    #     if not journal:
    ...
    #         'fiscal_position_id': (self.fiscal_position_id or self.fiscal_position_id._get_fiscal_position(partner_invoice_id)).id,
    ...
    def _prepare_invoice(self):
        ...
        invoice_vals = {
            ...
            'remarks_for_purchase' : self.remarks if self.remarks else False,
            'invoice_date':self.date_approve if self.company_id.backdate_for_bill else date.today(),
            'date':self.date_approve if self.company_id.backdate_for_bill else date.today(),
        }
        return invoice_vals
